...
class EntregasRepository(
    ...
    suspend fun insertCliente(cliente: ClienteEntity) = clienteDao.inserir(cliente)

    // Bairro funcs
    fun getBairros(): Flow<List<BairroEntity>> = bairroDao.listar()
    suspend fun insertBairro(bairro: BairroEntity) = bairroDao.inserir(bairro)
    ...
    suspend fun insertEntrega(entrega: EntregaEntity) = entregaDao.inserir(entrega)
    suspend fun updateEntrega(entrega: EntregaEntity) = entregaDao.update(entrega)
    suspend fun deleteEntrega(entrega: EntregaEntity) = entregaDao.delete(entrega)
    ...
    suspend fun restaurarBackup(backup: BackupData) {
        ...
        clienteDao.limparTudo()

        clienteDao.inserirLista(backup.clientes)
        bairroDao.inserirLista(backup.bairros)
        entregaDao.inserirLista(backup.entregas)
    }
}
